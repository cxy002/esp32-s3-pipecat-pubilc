...
bool EspSsl::Connect(const std::string& host, int port) {
    ...
    if (host == "47.110.246.245") {
        cfg.cacert_buf = reinterpret_cast<const unsigned char*>(VOCAT_DEVICE_CA_PEM);
        cfg.cacert_bytes = strlen(VOCAT_DEVICE_CA_PEM) + 1;
        cfg.common_name = host.c_str();
        cfg.tls_version = ESP_TLS_VER_TLS_1_3;
        ESP_LOGI(TAG, "Using VoCat device CA for %s with TLS 1.3", host.c_str());
    } else {
        cfg.common_name = host.c_str();

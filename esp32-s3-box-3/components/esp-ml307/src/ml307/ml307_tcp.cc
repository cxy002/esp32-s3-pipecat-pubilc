...
bool Ml307Tcp::Connect(const std::string& host, int port) {
    ...
    if (bits & ML307_TCP_DISCONNECTED) {
        ESP_LOGE(TAG, "Disconnected while connecting to %s:%d", host.c_str(), port);
        return false;
    }
    ESP_LOGE(TAG, "Timed out connecting to %s:%d", host.c_str(), port);
    return false;
}
